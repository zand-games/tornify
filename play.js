...
recognition.addEventListener("end", () => {
  if (txtRound.value > 0) recognition.start();
});
...
function lbltalkVisibile(val) {
  if (val) lbltalk.style.display = "inline";
  else lbltalk.style.display = "none";
}

function themeVisibility(val) {
  if (val) themeLable.style.visibility = "visible";
  else themeLable.style.visibility = "hidden";
}
